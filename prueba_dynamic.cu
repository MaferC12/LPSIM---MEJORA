...
void build_edge_owner_map_spatial(double split_x,
                                  const std::vector<Edge>& edges,
                                  const std::vector<Node>& nodes,
                                  std::vector<int>& h_edge_owner_map,
                                  int world_size)
{
    h_edge_owner_map.resize(edges.size());

    if (world_size == 1) {
        std::fill(h_edge_owner_map.begin(), h_edge_owner_map.end(), 0);
        return;
    }

    for (const auto& e : edges) {
        double x_u = nodes[e.u].x;
        if (x_u < split_x)
...
int main(int argc, char** argv) {
    ...
    const double IMBALANCE_THRESHOLD  = 0.20;
    const double MOVE_FRACTION        = 0.05;

    CUDA_CHECK(cudaSetDevice(0));

    if (rank == 0)
        std::cout << "=== Simulación MultiGPU MPI (Particionamiento Espacial Dinámico) ===\n"
                  << "Nodos: " << size << "\n";
    ...
    double min_x = 0.0, max_x = 0.0, split_x = 0.0;
    ...
    if (rank == 0) {
        ...
        std::cout << "DEBUG R0: Particionamiento Espacial inicial. Min X: "
                  << min_x << ", Max X: " << max_x
                  << ", Split X: " << split_x << "\n";

        build_edge_owner_map_spatial(split_x, edges, nodes, h_edge_owner_map, size);
        std::cout << "DEBUG R0: Mapeo de propietarios de aristas calculado (Inicial).\n";
    }

    MPI_Bcast(&min_x,  1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&max_x,  1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Bcast(&split_x, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    std::cout << "DEBUG R" << rank
              << ": Vehículos locales recibidos: "
              << local_veh.size() << "\n";
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        int migrated = exchange_and_update_gpu(d_veh, h_veh_buffer,
                                               rank, size,
                                               h_edge_owner_map);
        ...
        if (size == 2 && ((t + 1) % REBALANCE_INTERVAL == 0)) {
            int local_n = (int)d_veh.size();
            int counts[2] = {0, 0};
            ...
            int load0 = counts[0];
            int load1 = counts[1];
            int total = load0 + load1;
            double avg = (total > 0) ? (double)total / 2.0 : 0.0;

            if (avg > 0.0 && rank == 0) {
                double diff = std::abs(load0 - load1) / avg;

                if (diff > IMBALANCE_THRESHOLD) {
                    bool left_heavier = (load0 > load1);
                    double width = max_x - min_x;
                    double delta = MOVE_FRACTION * width;

                    if (left_heavier) {
                        split_x += delta;
                        if (split_x > max_x - 1e-6) split_x = max_x - 1e-6;
                        std::cout << "REBALANCEO: GPU0 sobrecargada ("
                                  << load0 << " vs " << load1
                                  << "). Nueva frontera X = " << split_x << "\n";
                    } else {
                        split_x -= delta;
                        if (split_x < min_x + 1e-6) split_x = min_x + 1e-6;
                        std::cout << "REBALANCEO: GPU1 sobrecargada ("
                                  << load1 << " vs " << load0
                                  << "). Nueva frontera X = " << split_x << "\n";
                    }

                    build_edge_owner_map_spatial(split_x, edges, nodes,
                                                 h_edge_owner_map, size);
                }
            }
            ...
            d_edge_owner_map = h_edge_owner_map;
        }

        if (t % 10 == 0 && rank == 0) {
            std::cout << "Step " << t
                      << " completado. Migraciones en este paso (R0): "
                      << migrated << "\n";
        }
    }
    ...
    if (rank == 0) {
        ...
        std::cout << "Total Migraciones en el Clúster: "
                  << global_migrations << "\n";
        std::cout << "(Este es el resultado clave para la Mejora 2: Particionamiento Espacial Dinámico)\n";
    }
