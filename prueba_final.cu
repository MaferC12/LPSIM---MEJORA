                              ...
                              LaneCell* d_next)
{
    ...
    if (i < n) {
        d_veh[i].update(d_edges, d_cur, d_next);
    }
}
...
inline int get_edge_owner(int edge_id, int world_size) {
    return edge_id % world_size;
}
                            ...
                            int my_rank, int world_size)
{
    ...
    for (const auto& v : h_veh_buffer) {
        ...
        int owner_rank = get_edge_owner(v.current_edge_id, world_size);
...
int main(int argc, char** argv) {
    ...
    if (rank == 0)
        std::cout << "=== Simulación MultiGPU MPI (Round-Robin por arista) ===\n"
                  << "Nodos: " << size << "\n";
    ...
    MPI_Bcast(edges.data(), num_edges * (int)sizeof(Edge),
    ...
    if (total_vehicles == 0) {
        if (rank == 0)
            std::cout << "No hay vehículos. Terminando.\n";
        MPI_Finalize();
        return 0;
    }
    ...
    for (int t = 0; t < steps; ++t) {
        ...
        int migrated = exchange_and_update_gpu(d_veh, h_veh_buffer, rank, size);
    ...
    if (rank == 0) {
        ...
        std::cout << "Total Migraciones en el Clúster: "
                  << global_migrations << "\n";
        std::cout << "(Este es el resultado clave para la Prueba 1)\n";
    }
