...
int main(int argc, char** argv) {
    ...
    if (rank == 0)
        std::cout << "=== Simulación MultiGPU MPI (Balanceo Estático por Capacidad de Arista) ===\n"
                  << "Nodos: " << size << "\n";
    ...
    if (rank == 0) {
        ...
        for (const auto& e : edges) {
            total_capacity += (long long)((int)e.length_meters * e.num_lanes);
        }

        long long target_capacity_per_rank = (size > 0) ? (total_capacity / size) : 0;
        long long current_rank_capacity = 0;
        int current_rank_idx = 0;

        for (const auto& e : edges) {
            h_edge_owner_map[e.id] = current_rank_idx;
            current_rank_capacity += (long long)((int)e.length_meters * e.num_lanes);

            if (current_rank_idx < size - 1 && current_rank_capacity >= target_capacity_per_rank) {
                current_rank_idx++;
                current_rank_capacity = 0;
            }
        }
        std::cout << "DEBUG R0: Mapeo de propietarios de aristas calculado (Balanceo Estático).\n";
    }
    ...
    if (rank == 0) {
        ...
        std::cout << "(Este es el resultado clave para la Prueba de Balanceo Estático)\n";
    }
